...
def get_processor(net, trainable_params, identifier, z_random, weights, optim, all_digits, forget_digit):
    ...
    def process_batch(img_retain, img_forget):
        ...
        gf = torch.cat([x.view(-1) for x in grad(outputs=loss_forget + 0.5 * uniformity_weight * uniformity, inputs=trainable_params, retain_graph=True)])
        gr = torch.cat([x.view(-1) for x in grad(outputs=loss_retain + 0.5 * uniformity_weight * uniformity, inputs=trainable_params, retain_graph=True)])
