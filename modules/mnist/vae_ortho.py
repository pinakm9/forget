...
def get_processor(net, trainable_params, identifier, z_random, weights, optim, all_digits, forget_digit):
    ...
    def process_batch(img_retain, img_forget):
        ...
        uniformity = vl.uniformity_loss(logits, digits)
        ...
        loss_forget = rec_forget + kl_weight * kl_forget + 0.5 * uniformity_weight * uniformity
        loss_retain = rec_retain + kl_weight * kl_retain + 0.5 * uniformity_weight * uniformity
        ...
        loss = loss_forget + loss_retain + orthogonality_weight * orth 
        optim.zero_grad()
        loss.backward()
        optim.step()
...
def train(model, folder, num_steps, batch_size, latent_dim=2, save_steps=None, collect_interval='epoch', log_interval=10,\
    ...
    process_batch = get_processor(net, ut.get_trainable_params(net), identifier, z_random, (kl_weight, uniformity_weight, orthogonality_weight), optim, all_digits, forget_digit)    
