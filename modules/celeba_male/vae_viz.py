...
def compare_model_outputs(model_a, model_b, num_samples=25, latent_dim=512, image_size=64):
    """
    ...
    # Generate the same latent vectors
    ...
    # Assume output shape (N, 3, H, W), convert to (N, H, W, 3) for imshow
    images_a = images_a.transpose(0, 2, 3, 1)
    images_b = images_b.transpose(0, 2, 3, 1)

    # Clip to valid range if outputs are not normalized
