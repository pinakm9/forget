...
def plot_transformation(images_a, images_b, index, save_path=None):
    images_a = images_a[index]
    images_b = images_b[index]
...
def plot_transformation2(images_a, images_b, index, titles_a=None, titles_b=None, save_path=None):
    """
    Plot original and transformed images side by side with custom titles.

    Args:
        images_a: numpy array of shape (N, C, H, W)
        images_b: numpy array of shape (N, C, H, W)
        index: indices of the samples to plot
        titles_a: list of strings for the top row (len must match len(index))
        titles_b: list of strings for the bottom row (len must match len(index))
    ...
    for idx in range(num_samples):
        axes[0, idx].imshow(images_a[idx])
        axes[0, idx].axis("off")
        if titles_a is not None:
            axes[0, idx].set_title(titles_a[idx], fontsize=10)

        axes[1, idx].imshow(images_b[idx])
        axes[1, idx].axis("off")
        if titles_b is not None:
            axes[1, idx].set_title(titles_b[idx], fontsize=10)

    # Add vertical labels to the left using fig.text
    fig.subplots_adjust(left=0.005, wspace=0.1, hspace=0.2)
    fig.text(0.0001, 0.7, "Original model", va='center', ha='left', rotation=90, fontsize=14)
    fig.text(0.0001, 0.3, "After unlearning", va='center', ha='left', rotation=90, fontsize=14)
...
def compare_model_outputs_2(images_a, images_b, index, save_path=None):
    images_a = images_a[index]
    images_b = images_b[index]
    num_samples = images_a.shape[0]
