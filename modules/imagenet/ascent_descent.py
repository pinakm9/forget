...
def get_processor_even(model, vae, diffusion, device, optim, trainable_params):
    device = vae.device
...
def get_processor_odd(model, vae, diffusion, device, optim):
    device = vae.device
    # amp = (device.type == "cuda")
    # scaler = torch.cuda.amp.GradScaler(enabled=amp)
    # @ut.timer
    def process_batch(img_r, label_r):
        # Forward pass
        start_time =  time.time()
        # net.eval()
    ...
    **gen_kwargs : dict, optional
    ...
    for _ in tqdm(range(1, epochs + 1), desc="Epochs"):
        for batch_retain, batch_forget in zip(dataloader['retain'], dataloader['forget']):
            ...
            if global_step % 2 == 1:
                loss, elapsed_time = process_batch_odd(img_retain, label_retain)
            else:
                loss, elapsed_time = process_batch_even(img_forget, label_forget)
